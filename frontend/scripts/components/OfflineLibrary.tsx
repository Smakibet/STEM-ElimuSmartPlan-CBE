...
const OfflineLibrary: React.FC<OfflineLibraryProps> = ({
  lessons,
  onView,
  onDelete,
}) => {
    ...
    <div className="h-full bg-white rounded-xl shadow-sm border border-slate-200 p-6 overflow-y-auto">
      <div className="mb-6 flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-500 text-sm">
            Access your lesson plans without internet
          </p>
        </div>
      ...
      {lessons.length === 0 ? (
        <div className="flex flex-col items-center justify-center h-64 text-slate-400 border-2 border-dashed border-slate-200 rounded-xl">
          ...
          <p className="text-sm">
            Generate and save lessons to see them here.
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {lessons.map((lesson) => (
            <div
              key={lesson.id}
              className="border border-slate-200 rounded-xl p-5 hover:shadow-lg transition-all group bg-white cursor-pointer"
              onClick={() => onView(lesson)}
            >
              <div className="flex justify-between items-start mb-2">
                <span className="text-xs font-bold text-emerald-600 bg-emerald-50 px-2 py-1 rounded uppercase tracking-wide">
                  {lesson.subject}
                </span>
