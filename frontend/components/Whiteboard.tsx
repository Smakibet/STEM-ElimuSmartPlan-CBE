...
import { DrawingTool } from '../scripts/types';
...
const Whiteboard: React.FC = () => {
  ...
  const stopDrawing = () => {
    if (isDrawing) {
      setIsDrawing(false);
      // For shapes, we haven't actually 'stroked' or 'filled' effectively in the draw loop for saving
      // Actually, for shapes, we re-drew them on every frame over the snapshot. 
      // The last draw call leaves the pixels there, so we just save history.
      saveHistory();
    }
  };
    ...
    <div className="flex flex-col h-full bg-slate-100 p-4 rounded-xl shadow-inner relative">
      <div className="absolute top-6 left-6 flex flex-wrap gap-2 bg-white p-2 rounded-lg shadow-md z-10 border border-slate-200 max-w-[90%]">
        <button
          ...
          className={`p-2 rounded hover:bg-slate-100 ${tool === DrawingTool.PEN ? 'bg-emerald-100 text-emerald-600' : 'text-slate-600'}`}
          title="Pen"
        >
        ...
        <button
          ...
          className={`p-2 rounded hover:bg-slate-100 ${tool === DrawingTool.ERASER ? 'bg-emerald-100 text-emerald-600' : 'text-slate-600'}`}
          title="Eraser"
        >
        ...
        <button
          ...
          className={`p-2 rounded hover:bg-slate-100 ${tool === 'RECT' ? 'bg-emerald-100 text-emerald-600' : 'text-slate-600'}`}
          title="Rectangle"
        >
        ...
        <button
          ...
          className={`p-2 rounded hover:bg-slate-100 ${tool === 'CIRCLE' ? 'bg-emerald-100 text-emerald-600' : 'text-slate-600'}`}
          title="Circle"
        >
        ...
        <button
          ...
          className={`p-2 rounded hover:bg-slate-100 ${tool === 'TEXT' ? 'bg-emerald-100 text-emerald-600' : 'text-slate-600'}`}
          title="Text"
        >
        ...
        <select
          ...
          className="bg-slate-50 border-none text-sm text-slate-600 focus:ring-0 rounded"
        >
